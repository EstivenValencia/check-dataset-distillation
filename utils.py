...
def get_dataset(dataset, data_path, batch_size=1, subset="imagenette", args=None):
    ...
    if dataset == 'CIFAR10':
    ...
    elif dataset.startswith('RICE'):
        print("CARGANDO DATASET ARROZ")
        data_path = "/data/home/rtabares/.juan/datatsets/Rice_Image_Dataset"

        # Calculo de la media y desviacion estandar

        # dataloader = DataLoader(dst_train, batch_size=5000, shuffle=False, num_workers=2)  # Procesar en lotes

        # # 3. Inicializar acumuladores
        # sum_pixels = torch.zeros(3)  # Acumulador para la media
        # sum_squared = torch.zeros(3)  # Acumulador para la varianza
        # num_pixels = 0  # Contador de píxeles

        # # 4. Recorrer dataset en lotes para calcular estadísticas por canal (R, G, B)
        # for images, _ in dataloader:
        #     batch_samples = images.size(0)  # Tamaño del batch
        #     images = images.view(batch_samples, 3, -1)  # Aplanar las imágenes a (batch, canales, pixeles)
            
        #     sum_pixels += images.mean(dim=[0, 2]) * batch_samples  # Media por canal, ponderada por el batch
        #     sum_squared += images.pow(2).mean(dim=[0, 2]) * batch_samples  # Cuadrado de la media
        #     num_pixels += batch_samples

        # # 5. Calcular la media y desviación estándar final
        # mean = sum_pixels / num_pixels
        # std = torch.sqrt(sum_squared / num_pixels - mean.pow(2))  # Desviación estándar con varianza
        ...
        num_classes = 5
        im_size = (128,128)
        mean = [0.1179, 0.1189, 0.1230]
        std = [0.2922, 0.2947, 0.3039]
        ...
        # Cargamos el dataset usando ImageFolder, que asume que cada subcarpeta es una clase
        ...
        train_split = int(0.8 * total)  # 80% para entrenamiento

        # Dividir los índices
        train_indices = indices[:train_split]        # 80% entrenamiento
        test_indices = indices[train_split:]           # 10% test
    ...
    if args.zca:
        ...
        args.zca_trans = zca
        torch.save(args.zca_trans.state_dict(), "zca_model.pth")
...
def evaluate_synset(it_eval, net, images_train, labels_train, testloader, args, return_loss=False, texture=False):
    ...
    for ep in tqdm.tqdm(range(Epoch+1)):
        ...
        if not os.path.exists('datos'):
            os.mkdir('datos')
        torch.save((images_train.cpu(), labels_train.cpu()), f'datos/train_data_{ep}_{uid}.pt')
        # Si el testloader se creó a partir de un TensorDataset, podemos extraer los tensores:
        test_images, test_labels = testloader.dataset.tensors
        torch.save((test_images.cpu(), test_labels.cpu()), f'datos/test_data_{ep}_{uid}.pt')
